...
public class Asteroid {
    ...
    void polygon(float x, float y, float radius, int npoints) {
        float angle = Constants.processing.TWO_PI / npoints;//set the angle between vertexes
        Constants.processing.beginShape();
        for (float a = 0; a < Constants.processing.TWO_PI; a += angle) {//draw each vertex of the polygon
            float sx = x + Constants.processing.cos(a) * radius;//math
            float sy = y + Constants.processing.sin(a) * radius;//math
            Constants.processing.vertex(sx, sy);
        }
    ...
    void move() {
        if (split) {//if split move the chunks
            ...
            pos.add(vel);//move it
            if (Constants.processing.isOut(pos)) {//if out of the playing area wrap (loop) it to the other side
                loopy();
            }
        }
    }
