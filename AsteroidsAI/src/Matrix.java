...
public class Matrix {
    ...
    void output() {
        for (int i =0; i<rows; i++) {
            for (int j = 0; j<cols; j++) {
                Constants.processing.print(matrix[i][j] + "  ");
            }
            Constants.processing.println(" ");
        }
        Constants.processing.println();
    }
    ...
    void randomize() {
        for (int i =0; i<rows; i++) {
            for (int j = 0; j<cols; j++) {
                matrix[i][j] = Constants.processing.random(-1, 1);
            }
        }
    }
    ...
    float sigmoid(float x) {
        float y = 1 / (1 + Constants.processing.pow((float) Math.E, -x));
        return y;
    }
    ...
    void mutate(float mutationRate) {
        ...
        for (int i =0; i<rows; i++) {
            for (int j = 0; j<cols; j++) {
                float rand = Constants.processing.random(1);
                if (rand<mutationRate) {//if chosen to be mutated
                    matrix[i][j] += Constants.processing.randomGaussian()/5;//add a random value to it(can be negative)
    ...
    Matrix crossover(Matrix partner) {
        ...
        int randC = floor(Constants.processing.random(cols));
        int randR = floor(Constants.processing.random(rows));
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
