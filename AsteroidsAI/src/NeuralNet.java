...
public class NeuralNet {
    ...
    NeuralNet getClone() {
        ...
        clone.whi = whi.getClone();
        clone.whh = whh.getClone();
        clone.woh = woh.getClone();
        return clone;
    }

    //converts the weights matrices to a single table
    //used for storing the NN in a file
    Table NetToTable() {
        ...
        for (int i = 0; i< Constants.processing.max(whiArr.length, whhArr.length, wohArr.length); i++) {
            t.addColumn();
        }
